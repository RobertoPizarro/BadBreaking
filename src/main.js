  ...
  lista.forEach((med) => {
    ...
    html += `<tr>
      ...
      med.precio_compra
                ...
                <button class="btn btn-danger" onclick="eliminarMedicamento(${
                  med.id_medicamento
...
async function guardarEdicionCompleta(e, id) {
  ...
  try {
    ...
    mostrarAlerta(div, "Error servidor", "error");
  }
}
...
async function eliminarMedicamento(id) {
  ...
  try {
    ...
    if ((await res.json()).estado === "exito") {
      alert("Desactivado");
      cargarMedicamentos();
    }
  } catch (e) {
...
async function procesarVenta() {
  ...
  try {
    ...
    if (data.estado === "exito") {
      mostrarAlerta(
        document.getElementById("resultadoVenta"),
        "Venta registrada",
        "success"
      );
      setTimeout(() => {
