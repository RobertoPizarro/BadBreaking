...
BEGIN
    ...
    v_ganancia := (v_precio_venta - v_precio_compra) * :NEW.cantidad;
    v_margen_porcentaje := ROUND(((v_precio_venta - v_precio_compra) / v_precio_venta) * 100, 2);
    
    -- Registrar en auditoría SI la ganancia es baja (margen menor al 20%)
    IF v_margen_porcentaje < 20 THEN
        INSERT INTO Auditoria_Medicamentos (
            ...
            :NEW.id_medicamento,
            v_nombre_med,
            'VENTA_MARGEN_BAJO: ' || v_margen_porcentaje || '% | Cant: ' || :NEW.cantidad,
            USER,
            SYSTIMESTAMP
        );
    END IF;
    ...
    SET stock = stock - :NEW.cantidad,
        estado = CASE 
            ...
            WHEN (stock - :NEW.cantidad) <= 5 THEN 'Activo'
            ELSE estado 
        END
    WHERE id_medicamento = :NEW.id_medicamento;
    
    -- Si el stock resultante es crítico (<=5), registrar alerta
    IF (v_stock_actual - :NEW.cantidad) <= 5 AND (v_stock_actual - :NEW.cantidad) > 0 THEN
        INSERT INTO Auditoria_Medicamentos (
            ...
            :NEW.id_medicamento,
            v_nombre_med,
            'ALERTA_STOCK_CRITICO: Quedan ' || (v_stock_actual - :NEW.cantidad) || ' unidades',
            USER,
            SYSTIMESTAMP
        );
    END IF;
...
BEGIN
    ...
    -- 5. Validar duplicado de lote (solo en INSERT)
    IF INSERTING THEN
    ...
    -- 6. Auto-completar estado según stock
    IF :NEW.stock <= 0 THEN
        :NEW.estado := 'Agotado';
...
BEGIN
    -- Determinar tipo de operación y construir mensaje detallado
    IF INSERTING THEN
        v_accion := 'REGISTRO_NUEVO';
        v_detalles := 'Stock inicial: ' || :NEW.stock || 
                     ' | Precio: S/.' || :NEW.precio_venta ||
    ...
    ELSIF UPDATING THEN
        -- Detectar cambios específicos y registrarlos
        
        -- Cambio de estado (Inactivo = eliminación lógica)
        IF UPDATING('estado') AND :NEW.estado = 'Inactivo' AND :OLD.estado != 'Inactivo' THEN
            v_detalles := 'Estado anterior: ' || :OLD.estado || 
                         ' | Stock remanente: ' || :OLD.stock ||
                         ' | Último precio: S/.' || :OLD.precio_venta;
            ...
            INSERT INTO Auditoria_Medicamentos (
                ...
                :OLD.id_medicamento,
                :OLD.nombre,
                'ELIMINACION_LOGICA - ' || v_detalles,
                USER,
                SYSTIMESTAMP
            );
        END IF;
        
        -- Cambio de precios (importante para análisis financiero)
        IF UPDATING('precio_compra') OR UPDATING('precio_venta') THEN
            v_detalles := 'Precio Compra: S/.' || :OLD.precio_compra || ' → S/.' || :NEW.precio_compra ||
                         ' | Precio Venta: S/.' || :OLD.precio_venta || ' → S/.' || :NEW.precio_venta ||
                         ' | Margen anterior: ' || ROUND(((:OLD.precio_venta - :OLD.precio_compra) / :OLD.precio_venta) * 100, 1) || '%' ||
                         ' | Margen nuevo: ' || ROUND(((:NEW.precio_venta - :NEW.precio_compra) / :NEW.precio_venta) * 100, 1) || '%';
            ...
            INSERT INTO Auditoria_Medicamentos (
                ...
                :NEW.id_medicamento,
                :NEW.nombre,
                'ACTUALIZACION_PRECIOS - ' || v_detalles,
                USER,
                SYSTIMESTAMP
            );
        END IF;
        
        -- Cambio significativo de stock (reposición o ajuste)
        IF UPDATING('stock') AND ABS(:NEW.stock - :OLD.stock) >= 10 THEN
            v_detalles := 'Stock anterior: ' || :OLD.stock || 
                         ' | Stock nuevo: ' || :NEW.stock ||
                         ' | Diferencia: ' || (:NEW.stock - :OLD.stock) ||
                         ' | Costo ajuste: S/.' || ROUND(ABS(:NEW.stock - :OLD.stock) * :NEW.precio_compra, 2);
    ...
    ELSIF DELETING THEN
        -- Eliminación física
        v_detalles := 'Stock eliminado: ' || :OLD.stock ||
